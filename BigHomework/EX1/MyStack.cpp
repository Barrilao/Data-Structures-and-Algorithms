...
class AdvancedStack {
    private:
        double stackArray[NMAX]; // an array of NMAX dimension
        int topLevel, topMax, topMin; // the top of the stack, representing the INDEX of last element of the
                    //stackArray:0, 1, 3,....
        double stackMax[NMAX];
        double stackMin[NMAX];

    public:
        void push(double x) // puts an element in the stack array
        {
			...
			if (x > stackMax[topMax] ){
            	stackMax[++topMax] = x;
            	int min = stackMin[topMin];
            	stackMin[++topMin] = min;
            }
            else if (x < stackMin[topMin]){
            	stackMin[++topMin] = x;	
            	int max = stackMax[topMax];
            	stackMax[++topMax] = max;
            }
        	else{
        		int max = stackMax[topMax], min = stackMin[topMin];
            	stackMax[++topMax] = max;
            	stackMin[++topMin] = min;
            }
        ...
        double pop() // extracts and element from the stack array and returns the new top
        {
            if (isEmpty())
            {
...
{ 
	...
	cout<< "Max: " << myStack.getMax() << " Min: " << myStack.getMin() <<"\n";
    return 0; 
} 
